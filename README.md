...
F0/5 будет настроен как транк, поэтому его принадлежность к VLAN 1 не имеет значения – транк пропускает все VLAN.
...
-Почему интерфейс F0/5 на S1 указан в VLAN 1?  
По умолчанию все порты находятся в VLAN 1. F0/5 будет настроен как транк, который пропускает все VLAN, а native VLAN будет изменена на 1000.  

-Какой IP-адрес получил бы ПК при подключении через DHCP?  
ПК получил бы первый свободный адрес из соответствующего пула.  
